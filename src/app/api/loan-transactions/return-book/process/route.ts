...
//       // Update availability status in bookcopy
...
//           // Nếu bị thất lạc, cập nhật availability_status
//           if (bookStatus.isLost) {
//             const { error: statusError } = await supabase
//               .from("bookcopy")
//               .update({ availability_status: "Thất lạc" })
//               .eq("copy_id", bookStatus.book.copy_id);

//             if (statusError) throw statusError;
//           } else {
//             // Nếu không thất lạc, đã trả về => cập nhật lại là "Có sẵn"
//             const { error: statusError } = await supabase
//               .from("bookcopy")
//               .update({ availability_status: "Có sẵn" })
//               .eq("copy_id", bookStatus.book.copy_id);
